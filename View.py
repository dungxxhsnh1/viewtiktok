...
class Main:
    def __init__(self):
        ...
        cursor.hide()
        txt=self.format(" VUI LÒNG NHÂP", " ID Video Tik Tok :  ")
        YourId=input(txt)
        self.videos = YourId
    ...
    def gui(self):
        ...
        txt = """\n██╗   ██╗██╗███████╗██╗    ██╗██████╗  ██████╗ ████████╗\n██║   ██║██║██╔════╝██║    ██║██╔══██╗██╔═══██╗╚══██╔══╝\n██║   ██║██║█████╗  ██║ █╗ ██║██████╔╝██║   ██║   ██║   \n╚██╗ ██╔╝██║██╔══╝  ██║███╗██║██╔══██╗██║   ██║   ██║   \n ╚████╔╝ ██║███████╗╚███╔███╔╝██████╔╝╚██████╔╝   ██║   \n  ╚═══╝  ╚═╝╚══════╝ ╚══╝╚══╝ ╚═════╝  ╚═════╝    ╚═╝\n             COPYRIGHT BELONG TO  LÊ CÔNG DỤNG  \n\n\n\n\n"""
    ...
    def title(self):
        ...
        while True:
            ...
            try:
                ...
                os.system(
                    f"title Lê Công Dụng © 2022  x  Zviews ^| Views: {views} ^| Elapsed Time: {curr_time} ^| v2.1"
                    if os.name == "nt"
                    else ""
                )
                time.sleep(0.5)
                ...
                os.system(
                    f"titel Lê Công Dụng © 2022  x  Zviews ^| Views: ERROR ^| Elapsed Time: {curr_time} ^| v2.1"
                    if os.name == "nt"
                    else ""
                )
                pass
    def solve_captcha(self, sessid):
        try:
            ...
            print(self.format(" ĐANG CHECK ",f" Hình ảnh xác thực đã giải quyết ! | {captcha_answer}"))
            return alpha_key
            ...
            print(
                self.format(
                    "!",
                    "Captcha LỖI | Check access to Zefoy | BẠN HÃY KHỞI ĐỘNG LẠI TOOL ",
                )
            )
            self.solve_captcha(sessid)
    def get_sessid(self):
        ...
        while True:
            try:
                ...
                if "This service is currently not working" in decryped_answer:
                    ...
                    print(self.format("THÔNG BÁO ",f" HÃY ĐỢI: {timer}     "), end="\r")
                    start = time.time()
                    while time.time() < start + timer:
                        print("\r", end="")
                        print(self.format("THÔNG BÁO ", f" HÃY ĐỢI: {round((start + timer) - time.time())}       "),end="\r")
                        time.sleep(1)
                    continue
                ...
                if latency > 3:
                    print(self.format("THÔNG BÁO ",f" ĐÃ GỬI LƯỢT XEM ĐẾN ID: {aweme_id}"))
...
class Menu:
    def format(self, symbol, text):
        ...
        return f"""   {Col.Symbol(symbol, self.lblue, self.blue)} {self.lblue}{text}{Col.reset}"""
    def main(self):
        banner=requests.get("https://raw.githubusercontent.com/dungxxhsnh1/bannerlogo/main/Logo").text
